...
/**
 * Manages a process-singleton embedded MongoDB instance for testing purposes, backed by flapdoodle.
 * Provides faster test execution and better isolation than relying on an external MongoDB service.
 *
 * <p>Configuration via system properties:</p>
 * <ul>
 *   <li>opencb.test.embeddedMongo - Enable/disable embedded MongoDB (default: true)</li>
 *   <li>opencb.test.mongodb.version - MongoDB version to use (default: 7.0)</li>
 *   <li>opencb.test.mongo.verbose - Enable verbose mongod stdout/stderr passthrough (default: false)</li>
 * </ul>
 *
 * <p>The embedded mongod is started lazily on the first {@link #start()} call, reused across the
 * entire JVM, and stopped via a shutdown hook. A single-member replica set {@code rs0} is
 * initialised so transactions are available.</p>
 */
public final class EmbeddedMongoDBManager {
    ...
    private EmbeddedMongoDBManager() {
        this.enabled = Boolean.parseBoolean(System.getProperty("opencb.test.embeddedMongo", "true"));
        this.mongoVersion = System.getProperty("opencb.test.mongodb.version", DEFAULT_MONGODB_VERSION);
        this.verbose = Boolean.parseBoolean(System.getProperty("opencb.test.mongo.verbose", "false"));
    }
    ...
    public synchronized void start() throws IOException {
        ...
        try {
            ...
            if (!verbose) {
                ...
                LOGGER.info("MongoDB output filtering enabled (use -Dopencb.test.mongo.verbose=true to see all logs)");
            }
    ...
    private static class FilteringPrintStream extends PrintStream {
        ...
        FilteringPrintStream(PrintStream original) {
            super(original);
        ...
        public void write(int b) {
            if (b == '\n') {
                ...
                boolean isMongodLog = line.contains("{\"t\":{\"$date\":")
                        || line.startsWith("[mongod output]")
                        || line.startsWith("[mongod error]");
                if (!isMongodLog) {
