...
public class MarkdownModelDoclet {
    ...
    private static Map<String, ClassDoc> classes = new HashMap<String, ClassDoc>();
    private static Set<ClassDoc> tablemodels = new HashSet<ClassDoc>();
    //  private static final Logger logger = LoggerFactory.getLogger(MarkdownModelDoclet.class);
    private static final Logger LOG = LoggerFactory.getLogger(MarkdownModelDoclet.class);
    ...
    public static boolean start(RootDoc rootDoc) {
        LOG.info("Generating markdown for the data model");
    ...
    private static Map<String, ClassDoc> createMap(ClassDoc[] classes) {
        LOG.info("Creating a Map with classes of the data model");
        Map<String, ClassDoc> res = new HashMap<String, ClassDoc>();
    ...
    public static void printDocument() {
        LOG.info("Printing markdowns representing the data model");
    ...
    private static String generateFieldsAttributesParagraph(FieldDoc[] fields, String className) {
        ...
        if (fields.length > 0) {
            for (int i = 0; i < fields.length; i++) {
                ...
                if (options.getTableTagsClasses().contains(className)) {
                    res.append("| " + fields[i].name() + " | " + stags.contains("unique") + " | "
                            + stags.contains("required") + " |" + stags.contains("required") + " | "
                            + stags.contains("internal") + " |\n");
                }
            }

            res.append("### Fields for Create Operations \n");
            res.append("`");
            for (String s : create) {
    ...
    private static String getFlag(boolean unique) {
        String res=String.valueOf(unique);
        res = "[logo]: img/" + res + ".png \"" + res + "\"";
    ...
    private static StringBuffer getTableModel(ClassDoc doc, FieldDoc[] fields, String fileName, StringBuffer res) {
        ...
        LOG.info("Generating tables of fields in the data model markdowns for class " + String.valueOf(doc));
        Set<ClassDoc> relatedTableModels = new HashSet<ClassDoc>();
        res.append("###" + fileName + "\n");
            ...
            for (int i = 0; i < fields.length; i++) {
                ...
                if (!isModel(String.valueOf(fields[i].type()))) {
                    res.append("| **" + (deprecated ? "~~" + fields[i].name() + "~~ <br> Deprecated" : fields[i].name())
                            + "**<br>*" + className + "*|" + constraints + " | "
                            + constructDescription(fields[i].commentText(), implNote, see) + " |\n");
                } else {
                    res.append("| **" + (deprecated ? "~~" + fields[i].name() + "~~ <br> Deprecated" : fields[i].name())
                            + "**<br>*<a href=\"" + fileName + ".md#" + className + "\"><em>" + className
                            + "</em></a>*| " + constraints + " | "
                            + constructDescription(fields[i].commentText(), implNote, see) + " |\n");
    ...
    private static String constructDescription(String commentText, String implNote, String see) {
        String res = commentText + "</br>" + implNote + "</br>" + renderSeeTag(see);
        return res;
    }
    ...
    public static boolean validOptions(String[][] options, DocErrorReporter reporter) {
        LOG.info("Validating input options");
        boolean res = Options.validOptions(options, reporter);
        Options.validOptions(options, reporter);
        LOG.info(res ? "Valid input options" : "Invalid input options");
        return res;
    }


    public static int optionLength(String option) {
        LOG.info("Validating input options " + option);
